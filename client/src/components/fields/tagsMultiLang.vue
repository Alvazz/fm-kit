        ...
        <b-tab-item v-for="(tab, i) in model[field.key]">
          <template slot="header">
            <span>
              ...
              {{tab.label | uppercase}}
              <span class="icon is-small is-right  is-clickable" @click="removeTab(tab.label)" v-show="false">
          ...
          <b-field>
            <b-taginput
            v-model="model[field.key][i].tags"
            :data="filteredTags"
            size="is-small"
            autocomplete
            :allow-new="allowNew"
...
export default {
  ...
  methods: {
    ...
    addTab(input) {
      const lang = { label: input, tags: [], default: false }
      console.log(lang)
